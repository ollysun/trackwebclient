...
$this->title = 'Admin: Distance Table';
$this->params['breadcrumbs'] = array(
    array(
        'label' => 'Admin',
        'url' => ['admin/']
    ),
    array('label'=> 'Distance Table')
);
?>
...
//						$can_focus = $hub_data_col_indexes[$hub['id']] != $x;
                        ...
                        <td data-payload='<?= json_encode($d); ?>' data-from="<?= $hub['id'] ?>"
                            ...
                            class="<?= $diagonal; ?><?= ($can_focus && !$not_set) ?
                                ...
                                'matrix_cell not_set'; ?>"><?= $can_focus ? (!empty( $d['length']) ? $d['length'] : 'N/S') : ''; ?></td>
                        <?php
...
<div class="modal fade" id="editTransitTimeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <form id="update_distance_mapping_form" class="">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Edit Distance Table Entry</h4>
                </div>
                <div class="modal-body row">
                    ...
                    <div class="form-group col-xs-3">
                        ...
                        <input id="zone_mapping_id" name="distance_id" type="hidden" class="form-control">
                    </div>
...
<?php $this->registerJsFile('@web/js/distance_table.js?v=1.1.0', ['depends' => [\app\assets\AppAsset::className()]]); ?>
